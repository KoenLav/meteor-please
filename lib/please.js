...
var PATH_TO_CHROME = path.resolve(__dirname, '../scripts');
...
var PATH_TO_CHROME_SCRIPT = path.resolve(PATH_TO_SCRIPTS, 'rh-chrome.sh');
...
please.prototype.chrome = function(){
	...
	console.log(PATH_TO_CHROME_SCRIPT,PATH_TO_CHROME);
	...
	var bash = spawn("bash", [PATH_TO_CHROME_SCRIPT], {cwd: this.config.app});
	...
	bash.on('close', function (code) {
		...
		fs.readdir(this.config.app,function(err,files){
